...
const csvPath = `${project}/data/processed/relative_strength_nasdaq100_optionable.csv`;
const xlsxPath = `${project}/data/processed/relative_strength_nasdaq100_optionable.xlsx`;
const previewPath = `${project}/data/processed/relative_strength_nasdaq100_optionable_preview.png`;
...
const rawRows = parseCsv(csvText.replace(/^\uFEFF/, ""));
...
const sheet = workbook.worksheets.add("Relative Strength");
...
sheet.getRange(`E2:G${rowCount}`).format.numberFormat = "0.00";
sheet.getRange(`H2:I${rowCount}`).format.numberFormat = "0.00";
...
sheet.getRange(`H2:H${rowCount}`).conditionalFormats.add("colorScale", {
  thresholds: ["min", "50%", "max"],
  colors: ["#FEE2E2", "#FEF3C7", "#DCFCE7"],
});

const widths = [60, 76, 120, 92, 90, 100, 130, 160, 170];
...
const inspect = await workbook.inspect({
  kind: "table",
  range: `Relative Strength!A1:${lastCol}12`,
