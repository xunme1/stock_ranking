...
DEFAULT_SOURCE = ROOT_DIR / "config" / "cn_stock_pool_source.csv"
DEFAULT_OUTPUT = ROOT_DIR / "config" / "cn_stock_pool.csv"

OUTPUT_COLUMNS = ["ticker", "name", "sector", "stock_type", "related_us"]
...
def build_pool(source_file: Path = DEFAULT_SOURCE) -> pd.DataFrame:
    ...
    for row in source.itertuples(index=False):
        ticker = normalize_cn_code(getattr(row, "股票代码", ""))
        ...
        rows.append(
            {
                "ticker": ticker,
                "name": clean_text(getattr(row, "股票名称", "")),
                "sector": sector or "未分类",
                "stock_type": stock_type or sector or "未分类",
                "related_us": clean_text(getattr(row, "关联美股", "")),
            }
        )
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Build normalized A-share stock pool from the curated CSV.")
...
def main() -> None:
    ...
    print(f"Built {len(df)} A-share tickers -> {args.output}")
