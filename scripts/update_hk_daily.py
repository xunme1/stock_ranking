...
def ticker_csv_path(ticker: str) -> Path:
    return RAW_HK_DAILY_DIR / f"{normalize_hk_code(ticker)}.csv"


def load_pool(path: Path = POOL_FILE, limit: int | None = None) -> list[str]:
    if not path.exists():
        raise FileNotFoundError(f"Hong Kong stock pool not found: {path}. Run scripts/build_hk_stock_pool.py first.")
    df = pd.read_csv(path, dtype={"ticker": str}).fillna("")
    tickers = [normalize_hk_code(item) for item in df["ticker"].tolist()]
...
def normalize_hist(df: pd.DataFrame, ticker: str) -> pd.DataFrame:
    column_map = {
        "日期": "date",
        "开盘": "open",
        "收盘": "close",
    ...
    for column in keep:
        if column not in result.columns:
            result[column] = pd.NA
    result = result[keep]
    result.insert(0, "ticker", normalize_hk_code(ticker))
...
def merge_old_new_data(ticker: str, new_df: pd.DataFrame) -> int:
    ...
    combined["ticker"] = combined["ticker"].map(normalize_hk_code)
...
def fetch_stock_daily(ak, ticker: str, start: date, end: date) -> pd.DataFrame:
    raw = ak.stock_hk_daily(symbol=normalize_hk_code(ticker), adjust="qfq")
    result = normalize_hist(raw, ticker)
    result["date_dt"] = pd.to_datetime(result["date"], errors="coerce")
    result = result[(result["date_dt"].dt.date >= start) & (result["date_dt"].dt.date <= end)].copy()
    return result.drop(columns=["date_dt"])


def fetch_index_daily(ak, start: date, end: date) -> pd.DataFrame:
    raw = ak.stock_hk_index_daily_sina(symbol=BENCHMARK_AK_SYMBOL)
    result = normalize_hist(raw, BENCHMARK_CODE)
    result["date_dt"] = pd.to_datetime(result["date"], errors="coerce")
    result = result[(result["date_dt"].dt.date >= start) & (result["date_dt"].dt.date <= end)].copy()
    return result.drop(columns=["date_dt"])
...
def update_one_ticker(
    ...
    for attempt in range(1, retries + 1):
        try:
            df = fetch_index_daily(ak, start, end) if ticker == BENCHMARK_CODE else fetch_stock_daily(ak, ticker, start, end)
            if df.empty:
                return "empty", 0
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--days", type=int, default=60, help="Lookback trading-day target for missing local CSVs.")
    ...
    parser.add_argument("--skip-benchmark", action="store_true", help="Do not update HSTECH benchmark.")
...
def main() -> None:
    ...
    if args.tickers:
        tickers = [normalize_hk_code(item) for item in args.tickers.split(",") if item.strip()]
    else:
