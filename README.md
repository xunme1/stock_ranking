...
After the US calendar refresh, the server preserves calendar-date snapshots so that a ticker remains eligible after Alpha Vantage advances to the next quarter. The three-day earnings-sentiment report is researched by a Codex weekday automation, validated locally as JSON, published through the dedicated `earnings-reports` Git branch, and synchronized into the server archive. This avoids using DeepSeek/Tavily as the daily report researcher. Reports are available in the earnings-calendar modal and at `/earnings-reports`; deployment and automation instructions are in `docs/codex_earnings_report_automation.md`.
